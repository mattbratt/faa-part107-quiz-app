const TestEngine = {
    ...
    startStandardTest() {
      this.startTest(60, false, 2 * 60 * 60);
    },
  
    startCustomTest(numQuestions, disableTimer) {
      this.startTest(numQuestions, disableTimer, 2 * 60 * 60);
    },
    ...
    renderQuestion() {
      ...
      const validAnswers = Object.values(this.answers).filter(v => v !== undefined && v !== null && v !== "").length;
      const progress = Math.floor((validAnswers / total) * 100);
  
      app.innerHTML = `
        <div class="progress mb-2" style="position:relative; height:25px;">
          <div class="progress-bar bg-success" style="width:${progress}%"></div>
          <div class="progress-bar-text">${validAnswers} of ${total} answered</div>
        </div>
        ...
        ${Object.entries(q.answers).map(([key, val]) =>
          `<div><label><input type="radio" name="answer" value="${key}" ${(this.answers[q.id] && this.answers[q.id] === key) ? "checked" : ""}> ${key}: ${val}</label></div>`
        ).join("")}
    ...
    showReviewScreen() {
      ...
      const total = this.currentTest.length;
      const validAnswers = Object.values(this.answers).filter(v => v !== undefined && v !== null && v !== "").length;
      const unanswered = total - validAnswers;
  
      app.innerHTML = `
        <h2>Review Answers</h2>
        <p>${validAnswers} answered, ${unanswered} unanswered</p>
        <ul>
          ${this.currentTest.map((q, idx) => {
            const status = (this.answers[q.id] && this.answers[q.id] !== "") ? "answered" : "unanswered";
